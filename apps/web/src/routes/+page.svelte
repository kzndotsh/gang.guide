<script lang="ts">
  ...
  import YearSliderPopover from '$lib/overlays/YearSliderPopover.svelte';
  ...
  import EdgeLegendPopover from '$lib/overlays/EdgeLegendPopover.svelte';
  ...
  const selectionAnnouncement = $derived(
    selectedNode ? `Selected ${orgDisplayTitle(selectedNode)}` : 'Selection cleared',
  );
  ...
  function onInspectorAnimationEnd(open: boolean) {
    if (!open) restoreMapInteraction();
  }
  ...
  function deselect() {
    selectedId = null;
    if (isMobile.current) restoreMapInteraction();
  }
...
<div class="fixed inset-0 flex h-dvh flex-col overflow-hidden bg-background pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]" style="background-color:#0d1117">
        ...
        <main class="relative min-h-0 flex-1 overflow-hidden bg-background">
          ...
          <div class="absolute top-3 right-3 left-3 z-[2] hidden items-center justify-between gap-2 md:flex">
            <button
              ...
              title="Search (⌘K)"
            >
              <Search class="size-3.5 shrink-0" strokeWidth={2} />
              <span class="text-[0.65rem]">Search…</span>
              <Kbd.Root class="ml-auto inline-flex">⌘K</Kbd.Root>
            </button>
            <YearSlider
              bind:yearMin
              bind:yearMax
