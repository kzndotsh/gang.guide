# IDEA.md: future sources and ideas

Ideas, data sources, and long-term goals that aren't actionable yet. Roadmap: [`TODO.md`](TODO.md).

---

### Already in the pipeline

Scrapers and/or extract runs exist for Chicago Gang History, UnitedGangs, DetroitStreetGangs, NGCRC, NYC Gangs, StoneGreasers, StopHoustonGangs, ADL, FBI NGTA (2009/2011/2015), InSight Crime, and SPLC. There is a Wikipedia scraper. Running extract on Wikipedia, SPLC, InSight Crime, and streetgangs is still on [`TODO.md`](TODO.md). Do not start new scrapers for those sites.
...
- [x] InSight Crime scraper (`insightcrime.py`)
- [x] SPLC Extremist Files scraper (`splc.py`)
- [x] FBI National Gang Threat Assessment PDFs (`fbi_ngta.py`)
- [x] ADL profiles / PDFs (`adl.py`)
- [x] NGCRC (`ngcrc.py`)
...
- [x] NGCRC scraper + 6-profile pipeline run (`ngcrc.py`). Extra profile URLs (GD, Latin Kings, BD, etc.) still useful.
...
- [x] **DetroitStreetGangs.com**: Detroit-specific sets and profiles (`dsg.py`)
...
- [x] Add `membership_estimate` to org schema (`membership_year` still open)
